...
form의 요소 `<input>`이 여러개일 경우 `<ul>`을 이용해서 묶어주기도 한다.

`<fieldset>`을 사용해서 form 요소들을 보기 쉽게 그룹으로 묶어줄 수도 있다.

`<legend>`는 `<fieldset>`으로 나누어진 구역에 제목을 붙일 때 사용한다.
...
웹 페이지의 내용 중 강조하고 싶은 부분이 있을 떄 사용한다.
...
`<head>`: title 이나 meta 등의 웹 페이지 정보가 들어감, 한번밖에 못씀.
...
`<input type="button">해서 만들어도 된다.` button은 컨텐츠를 포함할 수 있어서 시각적인 효과를 줄 수 있다.
